...
func OnlineMachine(ctx *gin.Context, chain *dbc.DbcChain) {
	...
	if err := chain.Report(types.MachineOnline, req.StakingType, req.ProjectName, req.MachineId); err != nil {
		rsp.Code = int(types.ErrCodeDbcChain)
		rsp.Message = err.Error()
		ctx.JSON(ohttp.StatusInternalServerError, rsp)
		return
...
func OfflineMachine(ctx *gin.Context, chain *dbc.DbcChain) {
	...
	if err := chain.Report(types.MachineOffline, req.StakingType, req.ProjectName, req.MachineId); err != nil {
		rsp.Code = int(types.ErrCodeDbcChain)
		rsp.Message = err.Error()
		ctx.JSON(ohttp.StatusInternalServerError, rsp)
		return
