...
// [2026-08-07 抖动逃罚上限] 累计推迟(宽限)时长的时间上限（2 专家审 SOUND）。取 30min：远大于正常重启恢复
//   (含无盘慢启动 ~9.5min)、又能在合理时间内落罚。★ 用「累计推迟时长」而非「重排次数」：次数上限会把
//   「先抖后死」的机器提前放弃推给 cron 兜底(会漏)，时间上限一路推迟到期限再判、不漏。
//   ★ 精确语义(勿误读)：firstDeferredAt 锚在**首次重排**(约掉线后 5min)而非掉线那刻 → 实际掉线→判罚 ≈ 35-40min。
//   ★ 真正被它抓住的 = 「对 DDN 持续连不上(半开死连接)、而后端因主服务侧仍在抖动一直返 recentGrace」的租用机
//   (旧行为=无限推迟逃罚)。**定期能完成一次 DDN 握手的机器会被 do.connect 清零 firstDeferredAt、不判**——这是
//   故意的 fail-open 残留(那种机器每轮确实短暂在线，宁可不罚也不误罚正在恢复的机器)，且不比 rearm 修复前更差。
